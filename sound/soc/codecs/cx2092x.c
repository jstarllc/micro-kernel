...
#define CX2092X_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S32_LE)

#define CX2092X_CAPE_ID(a, b, c, d)	(((a) - 0x20) << 8 | ((b) - 0x20) << 14 | ((c) - 0x20) << 20 | ((d) - 0x20) << 26)
...
struct cx2092x_cmd {
	int	num_32b_words:16;   	/* Indicates how many data to be sent. one word == 4 bytes. */
	u32	command_id:15;
	u32	reply:1;            	/* The device will set this flag once the operation is complete. */
	u32	app_module_id;
	u32	data[CX2092X_CMD_SIZE]; /* Used for storing parameters and receiving the returned data from device. */
};
...
void cx2092x_dump(struct device *dev, char *data, int length)
{
	int  index;
	char buff[3 * 60];
	...
	if(data == NULL) {
		...
		sprintf(buff, "invalid length");
	} else {
		for(index = 0; index < length; index++) {
			sprintf(&buff[index *3], "%02X ", data[index]);
		}	
	}
	
	dev_err(dev, "len = %d, data: %s\n", length, buff);
}
			   ...
			   struct cx2092x_cmd *cmd)
{
	...
	//cx2092x_dump(cx2092x->dev, (char *) i2c_data, ((num_32b_words * 4) + 2));

	if(num_32b_words <= 0) {
		dev_err(cx2092x->dev, "Invalid length. size = %d\n", num_32b_words);
		goto LEAVE;
	}

	/* calculate how many WORD that will be wrote to device*/
	cmd->num_32b_words = cmd->command_id & CX2092X_CMD_GET(0) ? CX2092X_CMD_SIZE : num_32b_words;

	/* write all command data except fo frist 4 bytes*/
	ret = regmap_bulk_write(cx2092x->regmap, 4, &i2c_data[1], size - 1);
	if (ret < 0) {
		dev_err(cx2092x->dev, "Failed to write command data. size = %d\n", size);
		goto LEAVE;
	}
	...
	if (cmd->reply == 1) {
		if (cmd->num_32b_words > 0) {
			/* check if there is returned data. If yes copy the returned data to cmd->data array */
			regmap_bulk_read(cx2092x->regmap, 8, &i2c_data[2], cmd->num_32b_words);
		    ...
		    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret <= 0) {
		...
		dev_dbg(codec->dev, "Current mode = %c%c%c%c\n",
			...
			ucontrol->value.bytes.data[2], ucontrol->value.bytes.data[3]);
		    ...
		    struct snd_ctl_elem_value *ucontrol)
{
	...
	cmd.num_32b_words = 1;
	cmd.data[0] = CX2092X_CAPE_ID(ucontrol->value.bytes.data[0], ucontrol->value.bytes.data[1],
		ucontrol->value.bytes.data[2], ucontrol->value.bytes.data[3]);
...
static const struct snd_kcontrol_new cx2092x_snd_controls[] = {
	CX2092X_CONTROL("SendCmd", cmd_info, cmd_get, cmd_put,
		SNDRV_CTL_ELEM_ACCESS_READ | SNDRV_CTL_ELEM_ACCESS_WRITE | SNDRV_CTL_ELEM_ACCESS_VOLATILE),
	CX2092X_CONTROL("Mode", mode_info, mode_get, mode_put,
		SNDRV_CTL_ELEM_ACCESS_READ | SNDRV_CTL_ELEM_ACCESS_WRITE | SNDRV_CTL_ELEM_ACCESS_VOLATILE),
};

static const struct snd_soc_dapm_widget cx2092x_dapm_widgets[] = {
	SND_SOC_DAPM_AIF_OUT("Mic AIF", "Capture", 0, SND_SOC_NOPM, 0, 0),
...
int cx2092x_dev_probe(struct device *dev, struct regmap *regmap)
{
	...
	/* GPIOs */
	//cx2092x->gpiod_reset = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_LOW);
	...
	ret = snd_soc_register_codec(cx2092x->dev, &soc_codec_driver_cx2092x,
		soc_codec_cx2092x_dai, ARRAY_SIZE(soc_codec_cx2092x_dai));
	if (ret < 0) {
		dev_err(dev, "Failed to register codec: %d\n", ret);
	} else {
		dev_err(dev, "%s: Register codec.\n", __func__);
	}
